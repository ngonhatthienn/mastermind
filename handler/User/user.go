...
func CreateUser(client *redis.Client, Fullname string, Username string, Password string, Email string, Role string) int32 { // in *pb.CreateUserRequest not very okay
	...
	_, _ = client.Set(context.Background(), share.UserPatternValue(strconv.Itoa(XId)), val, 0).Result()
